...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            if (parts.Length != 4) // Тепер вимагаємо 4 параметри!
            {
            ...
            var isWeighted = bool.Parse(parts[3]); // Зчитуємо вагу (true/false)
            BaseGradeBook gradeBook;

            if (type == "standard")
                gradeBook = new StandardGradeBook(name, isWeighted); // Передаємо вагу
            else if (type == "ranked")
                gradeBook = new RankedGradeBook(name, isWeighted); // Передаємо вагу
            else
            {
